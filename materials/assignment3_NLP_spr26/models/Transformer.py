...
class TransformerTranslator(nn.Module):
    ...
    def embed(self, inputs):
        ...
        # # word embeddings
        # word_emb = self.embeddingL(inputs)   # (N,T,H)

        # # position indices
        # positions = torch.arange(T, device=inputs.device).unsqueeze(0).expand(N, T)

        # # positional embeddings
        # pos_emb = self.posembeddingL(positions)  # (N,T,H)

        # # combine
        # embeddings = word_emb + pos_emb # (N,T,H)
        N, T = inputs.shape
        positions = torch.arange(T, device=inputs.device).unsqueeze(0).expand(N, T)
    ...
    def multi_head_attention(self, inputs):
        ...
        # scores1 = torch.matmul(Q1, K1.transpose(1, 2)) / math.sqrt(self.dim_k)   # (N, T, T)
        # attn_weights1 = self.softmax(scores1)                                     # (N, T, T)
        # head1 = torch.matmul(attn_weights1, V1)                                   # (N, T, dim_v)
        ...
        scores1 = torch.matmul(Q1, K1.transpose(1, 2)) / math.sqrt(self.dim_q)
        scores2 = torch.matmul(Q2, K2.transpose(1, 2)) / math.sqrt(self.dim_q)   # (N, T, T)

        attn1 = self.softmax(scores1)
        attn2 = self.softmax(scores2)

        head1 = torch.matmul(attn1, V1)
        head2 = torch.matmul(attn2, V2)
        ...
        # scores2 = torch.matmul(Q2, K2.transpose(1, 2)) / math.sqrt(self.dim_k)   # (N, T, T)
        # attn_weights2 = self.softmax(scores2)                                     # (N, T, T)
        # head2 = torch.matmul(attn_weights2, V2)                                   # (N, T, dim_v)

        # ----- Concatenate heads -----
        # multi_head = torch.cat([head1, head2], dim=2)                             # (N, T, dim_v * num_heads)
    ...
    def feedforward_layer(self, inputs):
        ...
        #############################################################################
        # outputs = None      #remove this line when you start implementing your code
        # x = self.embed(inputs)                  # (N,T,H)
        # x = self.multi_head_attention(x)        # (N,T,H)
        # x = self.feedforward_layer(x)           # (N,T,H)
        # outputs = self.final_linear(x)          # (N,T,output_size)
...
class FullTransformerTranslator(nn.Module):
    ...
    def __init__(self, input_size, output_size, device, hidden_dim=128, num_heads=2,
        ...
        self.srcembeddingL = nn.Embedding(self.input_size, self.hidden_dim)          # src tokens
        self.tgtembeddingL = nn.Embedding(self.output_size, self.hidden_dim)         # tgt tokens
        self.srcposembeddingL = nn.Embedding(self.max_length, self.hidden_dim)       # src positions
        self.tgtposembeddingL = nn.Embedding(self.max_length, self.hidden_dim)       # tgt positions
        ##############################################################################
        # TODO:
    ...
    def forward(self, src, tgt):
        ...
        # embeddings for src
        src_positions = torch.arange(src_T, device=src.device).unsqueeze(0).expand(N, src_T)
        src_emb = self.srcembeddingL(src) + self.srcposembeddingL(src_positions)

        # embeddings for tgt
        tgt_positions = torch.arange(tgt_T, device=tgt.device).unsqueeze(0).expand(N, tgt_T)
        tgt_emb = self.tgtembeddingL(tgt) + self.tgtposembeddingL(tgt_positions)

        # boolean causal mask for decoder
        tgt_mask = torch.triu(
            torch.ones(tgt_T, tgt_T, device=tgt.device, dtype=torch.bool),
            diagonal=1
        )

        # key padding masks
        src_key_padding_mask = (src == self.pad_idx)   # (N, src_T)
        tgt_key_padding_mask = (tgt == self.pad_idx)   # (N, tgt_T)
    ...
    def generate_translation(self, src):
        ...
        # # tensor to store logits
        # outputs = torch.zeros(N, T, self.output_size, device=src.device)

        # # initialize tgt with pad tokens
        # tgt = torch.full((N, T), self.pad_idx, dtype=torch.long, device=src.device)
        ...
        #     # greedy decoding
        #     next_token = torch.argmax(out[:, t, :], dim=1)

        #     # update tgt so next iteration can use prediction
        #     tgt[:, t] = next_token

        #     # save logits for current position
        #     outputs[:, t, :] = out[:, t, :]
        ...
        #     tgt[:, t] = torch.argmax(outputs[:, t, :], dim=1)
        ...
        # put <sos> into tgt before the loop
        tgt[:, 0] = src[:, 0]
        ...
        for t in range(T):
            ...
            # only update next position if it exists
            if t < T - 1:
                tgt[:, t + 1] = torch.argmax(out[:, t, :], dim=1)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
        return outputs
