...
class FullTransformerTranslator(nn.Module):
    ...
    def __init__(self, input_size, output_size, device, hidden_dim=128, num_heads=2,
        ...
        self.srcembeddingL = nn.Embedding(self.input_size, self.hidden_dim)          # source word embeddings
        self.tgtembeddingL = nn.Embedding(self.output_size, self.hidden_dim)         # target word embeddings
        self.srcposembeddingL = nn.Embedding(self.max_length, self.hidden_dim)       # source positional embeddings
        self.tgtposembeddingL = nn.Embedding(self.max_length, self.hidden_dim)       # target positional embeddings
        ##############################################################################
        # TODO:
    ...
    def generate_translation(self, src):
        ...
        for t in range(T):
            ...
            if t < T - 1:
                tgt[:, t + 1] = torch.argmax(out[:, t, :], dim=1) #predict next token and add it to tgt for the next time step
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
        return outputs
