...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout=0.2, model_type="RNN", attention=False):
        ...
        # 5) attention combine layer
        if self.attention:
            self.attention_combine = nn.Linear(
                emb_size + encoder_hidden_size,
                emb_size
            )
        #############################################################################
        #                              END OF YOUR CODE                             #
    ...
    def compute_attention(self, hidden, encoder_outputs):
        ...
        # expand hidden across time dimension: (N, 1, H) -> (N, T, H)
        hidden_expanded = hidden.expand(-1, encoder_outputs.size(1), -1)

        # cosine similarity over hidden dimension -> (N, T)
        ...
        # convert to probabilities across sequence length T
        attention_prob = torch.softmax(cosine_scores, dim=1).unsqueeze(1)  # (N, 1, T)
    ...
    def forward(self, input, hidden, encoder_outputs=None):
        ...
        embedded = self.dropout(embedded)         # (N, 1, emb_size)

        # 2) attention if enabled
        if self.attention:
            # for LSTM, hidden is a tuple: (h_n, c_n)
            if self.model_type == "LSTM":
                hidden_for_attention = hidden[0]   # use only h_n
            else:
                hidden_for_attention = hidden

            # attention weights: (N, 1, T)
            attention_prob = self.compute_attention(hidden_for_attention, encoder_outputs)

            # context vector: (N, 1, encoder_hidden_size)
            context = torch.bmm(attention_prob, encoder_outputs) # batch matrix multiplication

            # concatenate context and embedded input: (N, 1, encoder_hidden_size + emb_size)
            combined = torch.cat((context, embedded), dim=2)

            # project back to emb_size so it can enter RNN/LSTM
            rnn_input = self.attention_combine(combined)   # (N, 1, emb_size)
        else:
            rnn_input = embedded

        # recurrent step
        output, hidden = self.rnn(rnn_input, hidden)       # output: (N, 1, decoder_hidden_size)

        # 3) output layer + log softmax
        output = self.fc(output.squeeze(1))                # (N, output_size)
